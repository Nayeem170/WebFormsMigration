...
Phase 7 execution record (2026-09-14):

- Timeouts first, because they change what every other failure test measures. Frontend's two clients had been bare `new HttpClient()` (100s default); they now share `ServiceHttp`: 2s timeout, `SocketsHttpHandler` with 2-minute `PooledConnectionLifetime`, bounded retry (3 attempts, 200ms apart) for GET only. Orders' `CatalogClient` got the same handler and a 3s timeout; its reserve/release retry was already key-guarded. Wall-clock evidence: with service URLs pointed at a non-routable address the dashboard fails in 6.4s (3x2s plus delays) instead of 100s; with a stopped service it degrades in the same 6.4s. The pre-change stopped-service baseline was a blank 200 in ~4s (unhandled transport exception truncating the render); the literal minute-forty hang only appears against non-refusing endpoints.
- Retry policy split by method: GETs retry; product add/update/delete and all order writes stay single-shot because a timed-out create is an ambiguous outcome and a blanket retry double-adds; idempotency keys on product CRUD were considered and rejected as Phase 4-shaped work arriving two phases late. Failure testing found a real gap here: with Catalog down, `TaskCanceledException` (timeout) escaped the `HttpRequestException` catch and place-order returned 500 instead of the specified 502 `CatalogUnavailable`; the catch was broadened and the failure suite now proves the 502.
- Read-path degradation through one shared helper: `UiHelper.IsTransportFailure` (HttpRequestException or TaskCanceledException only, so domain rejections keep their pinned-message surfacing) and `UiHelper.ServiceUnavailableMessage`. Guards landed in the five named units (Default, StatCards, CategoryExpand, OrderHistory, OrdersTable) and, discovered while testing, in three more fetch sites that blank their whole pages before those controls bind: the OrderWizard product dropdown, the OrdersManage grid, and the Products page grid. Transport failure now renders a clear per-zone message with no stack trace and a bounded wall-clock.
- Connection-pool staleness addressed by the same `PooledConnectionLifetime` on every client; the restart checks in the failure suite kill each service by port and confirm Frontend recovers without its own restart.
- Correlation IDs across all three hops: `X-Correlation-ID` (constant in Contracts). Frontend mints one id per request in `HttpContext.Items`, attaches it on every outbound call, and logs it as a `[corr ...]` prefix on logger lines plus a per-request line from `Application_EndRequest`. Catalog and Orders read or mint the id in middleware, echo it on the response, and log each request with it to their own `App_Data/logs/app.log` via a minimal file-logger provider. Orders forwards the incoming id on reserve/release (not a fresh mint); a scripted wizard place-order produced the same id in all three logs: Frontend `POST /Pages/Orders/ -> 200`, Orders `POST /api/orders -> 201`, Catalog `POST /api/products/reserve -> 200`.
- Step 3 confirmed as satisfied without changes: all four apps bind `http://localhost:{8081,8082,8094,8095}`; no wildcard bindings exist.
- `scripts/test-failures.ps1` (18 checks, all green): invalid order/product payloads return 400 with messages; reserve replay is idempotent (unique per-run keys after a fixed literal broke the second run) with exact stock accounting; repeated product create yields two rows, documenting the specified non-idempotence; correlation ids reach and forward through both service logs and Frontend mints on page GETs; stopping Catalog degrades the dashboard to the shared message with no stack trace in 6.4s and place-order fails 502; both services restart from their built DLLs and Frontend recovers each time without a restart; stopping Orders degrades the dashboard and the API is unreachable. The script restarts services with `dotnet <dll>` from the project directory (no rebuild, avoiding bin locks); the stop-instances-to-build rule still applies when source changes.
- Verification: 18/18 failure checks; source-run and published-local smokes byte-for-byte unchanged from Phase 6 (including the 38 -> 35 -> 38 accounting and pinned insufficient-stock message); 21/21 characterization tests.
